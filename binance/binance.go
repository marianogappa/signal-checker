...
func BuildCandlestickIterator(input types.SignalCheckInput) func() (types.Candlestick, error) {
	return newBinanceCandlestickIterator(input).next
}
...
func (it *binanceCandlestickIterator) next() (types.Candlestick, error) {
	...
	klinesResult, err := getKlines(it.input.BaseAsset, it.input.QuoteAsset, it.requestFromMillis)
	if err != nil {
	...
	it.requestFromMillis = it.candlesticks[len(it.candlesticks)-1].Timestamp*1000 + 60
	return it.next()
}

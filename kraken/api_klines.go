...
func (r response) toCandlesticks() ([]common.Candlestick, error) {
	...
	if !ok {
		return []common.Candlestick{}, fmt.Errorf("data key [%v] did not contain an array of datapoints", dataKey)
	}

	candlesticks := make([]common.Candlestick, len(rawDataOuterArr))
	for i := 0; i < len(rawDataOuterArr); i++ {
		raw, ok := rawDataOuterArr[i].([]interface{})
		if !ok {
			return []common.Candlestick{}, fmt.Errorf("candlestick [%v] did not contain an array of data fields, instead: [%v]", i, rawDataOuterArr[i])
		}
		...
		candlestick := common.Candlestick{
			...
			OpenPrice:      common.JsonFloat64(kCandlestick.open),
			ClosePrice:     common.JsonFloat64(kCandlestick.close),
			LowestPrice:    common.JsonFloat64(kCandlestick.low),
			HighestPrice:   common.JsonFloat64(kCandlestick.high),
			Volume:         common.JsonFloat64(kCandlestick.volume),
